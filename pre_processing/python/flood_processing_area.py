...
balt_flood = balt_bg[["GISJOIN","GEOID","fid_1", "area"]].merge(bg_flood_area, on='GISJOIN')

#Multiply counts by pixel area. Divide flood area by BG area to get proportion of BG inundated
balt_flood.loc[:,~balt_flood.columns.isin(['GISJOIN', 'GEOID','fid_1', 'area'])] = (balt_flood.loc[:,~balt_flood.columns.isin(['GISJOIN', 'GEOID','fid_1', 'area'])] * pixel_area).div(balt_flood['area'], axis=0)


#Reorder columns so that surge level columns are in ascending order 
surge_cols = [float(x) for x in balt_flood.columns[4:]]
surge_cols.sort()
new_cols = ['GISJOIN'] + ['GEOID'] + ['fid_1'] + ['area'] + [str(x) for x in surge_cols]
balt_flood = balt_flood[new_cols]

#save dataframe to csv
balt_flood.to_csv(join(FO, 'model_inputs','surge_area_baltimore_base.csv'))

print("Base Files Complete")
...
#save dataframe to csv
balt_flood.to_csv(join(FO, "model_inputs",'surge_area_baltimore_levee.csv'))
